...
def load_cifar10(config, with_embeddings=False, return_indices=False):
    ...
    train_set = datasets.CIFAR10(root='./data/raw', train=True, download=True)
    test_set = datasets.CIFAR10(root='./data/raw', train=False, download=True)
    ...
    train_labels = np.array(train_set.targets).astype(np.float32)
    test_labels = np.array(test_set.targets).astype(np.float32)
    ...
    if arch_type == 'conv':
        ...
        val_data = val_data.reshape(-1, 3072)
